...
```bash
git clone https://github.com/deShal3v/psth-explorer.git
cd psth-explorer
./setup.sh
```
...
switches array, picks a condition, changes view mode, or presses play. Those POST to
`/collect`, a Pages Function in `functions/collect.js` that writes to Analytics
Engine. Being a Pages Function it deploys with the site and runs on the same origin,
so there is no separate Worker, no CORS, and no third-party request for an ad blocker
to object to.

Analytics Engine has to be enabled once for the account, under Workers and Analytics
Engine in the dashboard. Then attach the binding:

```bash
curl -X PATCH "https://api.cloudflare.com/client/v4/accounts/$CF_ACCOUNT/pages/projects/psth" \
  -H "Authorization: Bearer $CF_TOKEN" -H "Content-Type: application/json" \
  --data '{"deployment_configs":{"production":{"analytics_engine_datasets":{"NSE_EVENTS":{"dataset":"psth"}}}}}'
```

Until that binding exists the endpoint accepts events and discards them, so nothing
breaks. Query what it collects with SQL:
...
Set `window.NSE_ANALYTICS_URL = ""` in `index.dc.html` to switch interaction events
off entirely.
...
| T12, T15, T16, T17 | Kunz et al. 2025, *Inner speech in motor cortex and implications for speech neuroprostheses* | [10.5061/dryad.gf1vhhn1j](https://doi.org/10.5061/dryad.gf1vhhn1j) |
...
| `functions/collect.js` | Pages Function collecting in-app events into Analytics Engine |
