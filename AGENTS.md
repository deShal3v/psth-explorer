...
The public deployment is Cloudflare Pages (project `psth`, <https://psth.io>), a direct
upload of the app plus `data/`. Nothing is served from a personal machine, and there is
no build step on Cloudflare.
...
That downloads the prebuilt cubes (about 25 MB, five participants) and serves on port 8099.
...
Only the `.u8.gz` sidecars are distributed; the raw `.u8` is gitignored and absent
from the release tarball. Hosts disagree about how to serve them, so `fetchCube()`
always requests the `.gz` and decides what to do from the bytes: if they still start
with the gzip magic `1f 8b` it inflates them with `DecompressionStream`, and if the
transport already inflated them it uses them as-is. That is what lets the same build
run on Cloudflare Pages (serves `.gz` as `application/gzip`, no `Content-Encoding`)
and on `tools/serve_gz.py` locally. Do not "simplify" it back to fetching `.u8`; on
Pages that file does not exist.

Locally, prefer `tools/serve_gz.py` over `python3 -m http.server`. It also answers a
request for `foo.u8` with the contents of `foo.u8.gz`, which keeps older links working.
...
| `functions/collect.js` | Pages Function receiving `track()` interaction events |
| `_headers` | Cloudflare Pages caching rules; `/data/*` is immutable, the app is not |
